...
public class JHLabsPlugin extends AbstractPlugin<BufferedImageOp>{
	...
	@Override
	protected BufferedImage getImage(BufferedImage image, BufferedImageOp t,List<Parameter> list) {
		setFields(t);
		return t.filter(image,null);
	}
	@Override
	protected LinkedHashMap<BufferedImageOp, List<Parameter>> getMap() {
		LinkedHashMap<BufferedImageOp, List<Parameter>> map = new LinkedHashMap<BufferedImageOp, List<Parameter>>();
		...
		for(BufferedImageOp op : biOps){
			map.put(op,new ArrayList<Parameter>());
