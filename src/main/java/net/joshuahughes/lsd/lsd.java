...
//	#include "lsd.h"
	...
	public static final char NOTDEF = (char)-1024;
	
	/** 3/2 pi */
	public static final double M_3_2_PI = 4.71238898038;

	/** 2 pi */
	public static final double M_2__PI = 6.28318530718;

	/** Label for pixels not used in yet. */
	public static final char NOTUSED = 0;
...
//	  System.exit(EXIT_FAILURE);
	 ...
	 */
	public static final double RELATIVE_ERROR_FACTOR = 100.0;

	public static final int EXIT_FAILURE = 1;
	public static final double DBL_MIN = Double.MIN_NORMAL;
	public static final double DBL_EPSILON = 2.2204460492503131e-16;

	private static final double UINT_MAX = Integer.MAX_VALUE;
...
//	  free( (void *) in );
	...
	static ntuple_list new_ntuple_list(int dim)
	{
	  ...
	  n_tuple.values = new double[dim*n_tuple.max_size];
	...
	static void enlarge_ntuple_list(ntuple_list n_tuple)
	{
	  ...
	  /* realloc memory */
	  n_tuple.values = new double[n_tuple.dim * n_tuple.max_size];
	  if( n_tuple.values == null ) error("not enough memory.");
	}
...
//	  free( (void *) i );
	 ...
	 */
	static image_char new_image_char_ini( int xsize, int ysize,
	                                      char fill_value )
	{
	 ...
	 */
	static image_int new_image_int(int xsize, int ysize)
	{
	  image_int image;
	...
	/** Create a new image_int of size 'xsize' times 'ysize',
	    initialized to the value 'fill_value'.
	 */
	static image_int new_image_int_ini( int xsize, int ysize,
	                                    int fill_value )
	{
...
//	  free( (void *) i );
	 ...
	 */
	static image_double new_image_double(int xsize, int ysize)
	{
	  image_double image;
	 ...
	 */
	static image_double new_image_double_ptr( int xsize,
	...
	static image_double ll_angle( image_double in, double threshold,
	                              coorlist list_p, coorlist [] mem_p,
...
//	  modgrad = new_image_double(in.xsize,in.ysize);
	  modgrad.data = new double[in.xsize*in.ysize];
	  /* get memory for "ordered" list of pixels */
	  list = new coorlist[n*p];for(int index=0;index<list.length;index++)list[index] = new coorlist();
	  mem_p = list;
...
//	  free( (void *) range_l_e );
	 ...
	 */
	static boolean isaligned( int x, int y, image_double angles, double theta,
	                      double prec )
	{
	  double a;
	 ...
	 */
	public static final int TABSIZE = 100000;

	private static final double DBL_MAX = 1.7976931348623157e+308;
...
//	  free( (void *) iter );
	 ...
	 */
	static boolean ri_end(rect_iter  i)
	{
	  /* check input */
	...
	/** Create and initialize a rectangle iterator.
	    ...
	    See details in \ref rect_iter
	 */
	static rect_iter ri_ini(rect r)
	{
	 ...
	 */
	static double rect_nfa(rect rec, image_double angles, double logNT)
	{
	  rect_iter i;
	  int pts = 0;
	  int alg = 0;
	 ...
	 */
	static void region_grow( int x, int y, image_double angles, point[] reg,
	                         Int reg_size, Dbl reg_angle, image_char used,
	                         double prec )
	{
	  double sumdx,sumdy;
	  int xx,yy,i;
	  ...
	  reg[0].x = x;
	  reg[0].y = y;
	  reg_angle.x = angles.data[x+y*angles.xsize];  /* region's angle */
	  sumdx = Math.cos(reg_angle.x);
	  sumdy = Math.sin(reg_angle.x);
	  used.data[x+y*used.xsize] = USED;

	  /* try neighbors as new region points */
	  for(i=0; i<reg_size.x; i++)
	    for(xx=reg[i].x-1; xx<=reg[i].x+1; xx++)
	      for(yy=reg[i].y-1; yy<=reg[i].y+1; yy++)
	        if( xx>=0 && yy>=0 && xx<(int)used.xsize && yy<(int)used.ysize &&
	            used.data[xx+yy*used.xsize] != USED &&
	            isaligned(xx,yy,angles,reg_angle.x,prec) )
	          {
	            /* add point */
	            used.data[xx+yy*used.xsize] = USED;
	            reg[reg_size.x].x = xx;
	            ...
	            reg_angle.x = Math.atan2(sumdy,sumdx);
	          }
	}
	 ...
	 */
	static double rect_improve( rect  rec, image_double angles,
	 ...
	 */
	static boolean reduce_region_radius( point [] reg, Int reg_size,
	                                 image_double modgrad, double reg_angle,
	                                 double prec, double p,rect rec,
	  ...
	  while( density < density_th )
	    {
	        ...
	        if( dist( xc, yc, (double) reg[i].x, (double) reg[i].y ) > rad )
	          {
	            ...
	            reg[i].x = reg[reg_size.x-1].x; /* if i==*reg_size-1 copy itself */
	            reg[i].y = reg[reg_size.x-1].y;
	            --(reg_size.x);
	      ...
	      if( reg_size.x < 2 ) return FALSE;

	      /* re-compute rectangle */
	      region2rect(reg,reg_size.x,modgrad,reg_angle,prec,p,rec);
	 ...
	 */
	static boolean refine( point [] reg, Int reg_size, image_double modgrad,
	                   Dbl reg_angle, double prec, double p,rect rec,
	                   image_char used, image_double angles, double density_th )
	{
	  ...
	  /* re-compute rectangle */
	  region2rect(reg,reg_size.x,modgrad,reg_angle.x,prec,p,rec);
	  ...
	  if( density < density_th )
	    return reduce_region_radius( reg, reg_size, modgrad, reg_angle.x, prec, p,
	                               ...
	                               int [] reg_img, Int reg_x, Int reg_y )
	{
	  ...
	  coorlist[] mem_p = new coorlist[1];
	  rect rec = new rect();
	  point [] reg;
	  Int reg_size = new Int();int min_reg_size,i;
	  int xsize,ysize;
	  Dbl reg_angle =new Dbl();double rho,prec,p,log_nfa,logNT;
	  ...
	  */
	  ...
	  reg = new point[xsize*ysize];for(int index=0;index<reg.length;index++) reg[index] = new point();
	  ...
	  for(; list_p != null; list_p = list_p.next )
	    if( used.data[ list_p.x + list_p.y * used.xsize ] == NOTUSED &&
	          ...
	          because we are only interested in the exact NOTDEF value */
	      {
	        ...
	        if( reg_size.x < min_reg_size ) continue;
	  ...
	  if( reg_img != null && reg_x != null && reg_y != null )
	    {
	      ...
	      reg_img = region.data;
	      if( region.xsize > (int) INT_MAX ||
	          region.xsize > (int) INT_MAX )
	        error("region image to big to fit in INT sizes.");
	      reg_x.x = (int) (region.xsize);
	      reg_y.x = (int) (region.ysize);
...
//	      free( (void *) region );
	  ...
	  if( out.size > (int) INT_MAX )
	    error("too many detections to fit in an INT.");
	  n_out.x = (int) (out.size);
...
//	                            as a result. */
	...
	/*----------------------------------------------------------------------------*/
		...
		public static void main(String[] args) throws Exception{
			BufferedImage image = ImageIO.read(new File("chairs.png"));
			JOptionPane.showInputDialog(new ImageIcon(image));
			Int n_out = new Int();
			double[] result = lsd(n_out,imageToArray(image),image.getWidth(),image.getHeight());
			System.err.flush();
			System.err.println(result.length+"\t"+n_out.x);
		}
		...
		private static double[] imageToArray(BufferedImage image) {
			...
			for(int x=0;x<image.getWidth();x++)
				for(int y=0;y<image.getHeight();y++){
					...
					array[index++] = (color.getRed()+color.getGreen()+color.getBlue())/(3d);
				}
